...
app.post('/api/user/register', async (req, res) => {
  try {
    ...
    if (users[emailLower]) return res.status(409).json({ error: 'Este email ya está registrado' });
    users[emailLower] = {
      email: emailLower,
      name: name || emailLower.split('@')[0],
      plan: 'free',
      createdAt: new Date().toISOString(),
      generations: {},  // { 'YYYY-MM': count }
    };
    saveUsers(users);
    const token = jwt.sign(
    ...
    if (process.env.SMTP_USER) {
      try {
        ...
        await transporter.sendMail({
          from: `"${process.env.FROM_NAME||'CourseAI'}" <${process.env.SMTP_USER}>`,
          to: emailLower,
          subject: '✦ Bienvenido a CourseAI',
          html: `<div style="font-family:sans-serif;background:#0d0d0f;color:#f0efe8;padding:32px;border-radius:16px;max-width:500px;margin:0 auto">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:24px">
              <div style="width:32px;height:32px;border-radius:8px;background:#c8f060;display:inline-flex;align-items:center;justify-content:center;font-weight:bold;color:#000;font-size:16px">✦</div>
              <span style="font-size:18px;font-weight:500">CourseAI</span>
            </div>
            <h2 style="margin:0 0 12px;font-size:20px;color:#f0efe8">¡Bienvenido, ${users[emailLower].name}! 🎉</h2>
            <p style="color:#9b9a94;line-height:1.7;margin:0 0 16px">Tu cuenta fue creada exitosamente. Con el plan <strong style="color:#c8f060">Free</strong> podés generar <strong style="color:#f0efe8">3 cursos por mes</strong> sin costo.</p>
            <p style="color:#9b9a94;line-height:1.7;margin:0">Cuando estés listo para escalar, podés pasar a Starter, Pro o Agencia desde el panel de pago.</p>
            <div style="margin-top:24px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.08);font-size:12px;color:#5a5955">courseai.click</div>
          </div>`
        });
      } catch(e) { console.error('Error email bienvenida:', e.message); }
    }
    res.json({ ok: true, token, user: { email: emailLower, name: users[emailLower].name, plan: 'free', generationsLeft: 3 } });
  } catch(err) {
    res.status(500).json({ error: err.message });
  }
});

// Login de usuario (solo email — sin contraseña, magic link simple)
app.post('/api/user/login', async (req, res) => {
  try {
    ...
    const emailLower = email.toLowerCase().trim();
    const users = loadUsers();
    if (!users[emailLower]) return res.status(404).json({ error: 'Email no registrado. ¿Ya te creaste una cuenta?' });
    const token = jwt.sign(
      { email: emailLower, role: 'user' },
      process.env.JWT_SECRET || 'courseai-secret-change-me',
      { expiresIn: '30d' }
    ...
    const used = users[emailLower].generations[month] || 0;
    const limit = users[emailLower].plan === 'free' ? 3 : 999;
    res.json({ ok: true, token, user: { email: emailLower, name: users[emailLower].name, plan: users[emailLower].plan, generationsLeft: Math.max(0, limit - used) } });
  } catch(err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/user/me', (req, res) => {
  ...
  try {
    ...
    const month = currentMonth();
    const used = u.generations[month] || 0;
    const limit = u.plan === 'free' ? 3 : 999;
    res.json({ email: u.email, name: u.name, plan: u.plan, generationsUsed: used, generationsLeft: Math.max(0, limit - used), generationsLimit: limit });
  } catch {
    res.status(401).json({ error: 'Token inválido' });
  }
});

// Registrar una generación usada
app.post('/api/user/use-generation', (req, res) => {
  const auth = req.headers.authorization;
  if (!auth?.startsWith('Bearer ')) return res.status(401).json({ error: 'No autenticado' });
  try {
    const decoded = jwt.verify(auth.slice(7), process.env.JWT_SECRET || 'courseai-secret-change-me');
    ...
    const used = u.generations[month] || 0;
    const limit = u.plan === 'free' ? 3 : 999;
    if (u.plan === 'free' && used >= limit) {
      return res.status(403).json({ error: 'Límite del plan Free alcanzado. Pasá a Starter para continuar.' });
    }
    u.generations[month] = used + 1;
    saveUsers(users);
    res.json({ ok: true, generationsUsed: u.generations[month], generationsLeft: Math.max(0, limit - u.generations[month]) });
  } catch {
    res.status(401).json({ error: 'Token inválido' });
  }
});
...
app.get('/api/users', requireAuth, (req, res) => {
  ...
  const list = Object.values(users).map(u => ({
    ...
    generationsThisMonth: u.generations[month] || 0
  }));
